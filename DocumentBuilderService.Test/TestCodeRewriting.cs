...
{
	[TestClass]
	public class TestCodeRewriting
	{
		...
		public void TestLogFileCreated()
		{
			...
			Thread.Sleep(10);
			_fileService.Stop();
			Assert.IsTrue(File.Exists(@"CodeRewritingMethodLog.txt"));
		}
		...
		public void TestLogFileConsistent()
		{
			...
			Thread.Sleep(10);
			_fileService.Stop();
			var jsons = File.ReadAllText(@"CodeRewritingMethodLog.txt").TrimEnd(Convert.ToChar(";")).Split(Convert.ToChar(";"));
		...
		private void DeleteOutFiles()
		{
			...
			if (File.Exists("CodeRewritingMethodLog.txt"))
			{
				File.Delete("CodeRewritingMethodLog.txt");
			}
		}
	}
}
